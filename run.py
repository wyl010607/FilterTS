...
parser = argparse.ArgumentParser(description='FilterTS')

# basic config
parser.add_argument('--seed', type=int, default=2021, help='fix_seed')
...
if args.is_training:
    ...
    search_='quantile{}_bandwidth{}_top_K_static_freqs{}_filter_type{}'.format(
        args.quantile,
        args.bandwidth,
        args.top_K_static_freqs,
        args.filter_type)
    end = time.time()
    used_time = end -start
    print("time:",used_time)
    f = open("result.txt", 'a')
    f.write('search_:{}'.format(search_))
